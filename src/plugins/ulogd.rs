...
static RE_FIELD: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(IN|OUT|MAC|TTL|SRC|DST|TOS|PREC|SPT|DPT|PROTO|LEN|ID|DF|WINDOW|RES|SYN|URGP)=(\S*)").unwrap()
});

pub struct Ulogd;

impl Plugin for Ulogd {
    fn name(&self) -> &'static str { "ulogd" }
    fn kind(&self) -> PluginType { PluginType::Partial }
    ...
    fn process(&self, line: &str) -> PluginResult {
        ...
        let result = RE_FIELD.replace_all(line, |caps: &regex::Captures<'_>| {
            ...
            format!("{}={}", colorize(Color::Field, key), colorize(val_color, val))
        });
