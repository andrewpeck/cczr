...
static RE_IP: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b(?:\d{1,3}\.){3}\d{1,3}\b").unwrap());
...
static RE_DIR: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?:^|\s)((?:/[^\s/]+)+/?)").unwrap());
static RE_SIZE: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\b\d+(?:\.\d+)?[KMGT]?B\b").unwrap());
static RE_VERSION: Lazy<Regex> = Lazy::new(|| Regex::new(r"\bv?\d+\.\d+(?:\.\d+)*\b").unwrap());
static RE_TIME: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b\d{2}:\d{2}:\d{2}\b").unwrap());
static RE_ADDRESS: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b0x[0-9a-fA-F]+\b").unwrap());
...
pub fn colorize_words(input: &str) -> String {
    ...
    let add_spans =
        |re: &Regex, color: Color, text: &str, spans: &mut Vec<(usize, usize, Color)>| {
